...
export async function SCIENTIA_ES_LUX_PRINCIPIUM(
  ...
  const listOfQuoteArray = symbolIDsListMappable(getQuotesMapper);
  const listOfQuote = flatMapFirstElement(listOfQuoteArray);
  //
  const quotesMapper = applyMappingList(getQuotesMapper);
  const someval1 = quotesMapper(symbolIDsList);
  //
  //
  void someval1, listOfQuote;
  // const someConverted = typeCorrection(listOfQuoteArray);
  // void someConverted;
  ...
  void getSymbolsMapper, someval1, listOfQuoteArray;
